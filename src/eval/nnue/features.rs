use crate::eval::nnue::constants::FEATURE_DIMS;
#[cfg(test)]
use crate::eval::nnue::compat::{Board, Position, PositionExt};
use crate::eval::nnue::compat::{Color, ColorExt, EMPTY_SQUARE, Piece, Square, color_from_code, piece_from_code};
...
pub fn enumerate_active_features(
    position: &Position,
    out: &mut [[u32; crate::eval::nnue::constants::MAX_ACTIVE_FEATURES]; 2],
    lengths: &mut [usize; 2],
) -> usize {
    lengths.fill(0);
    ...
    debug_assert!(lengths[0] <= crate::eval::nnue::constants::MAX_ACTIVE_FEATURES);
    lengths[0]
}
...
fn push_perspective_features(
    ...
    perspective: Color,
    king_square: Square,
    out: &mut [u32; crate::eval::nnue::constants::MAX_ACTIVE_FEATURES],
    len: &mut usize,
) {
    for raw in 0u8..64 {
        let square = Square::from_raw(raw);
        let piece_code = board.piece_code_at(square);
        if piece_code == EMPTY_SQUARE {
            continue;
        }

        out[*len] = feature_index_from_piece_code(perspective, piece_code, square, king_square);
        *len += 1;
    }
}
...
mod tests {
    ...
    use crate::eval::nnue::constants::MAX_ACTIVE_FEATURES;
    use crate::eval::nnue::compat::{Color, Piece, Position, PositionExt, Square, encode_piece};
    ...
    fn direct_feature_index_matches_startpos_white_king_piece() {
        ...
        assert!(index < crate::eval::nnue::constants::FEATURE_DIMS as u32);
    }
}
